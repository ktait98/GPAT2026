...
CONTAINS
    ...
    SUBROUTINE DERIV(Y, DJ, RC, FL, DTS)
        ...
        ! Keep YP fixed as the timestep-start state
        YP(:,:) = Y(:,:)

        DO ITER = 1, NIT

            ! DO NOT reset full YP here.
            ! This must stay fixed through the NIT sweeps to match boxm_orig.
    ...
    SUBROUTINE RUN_LEGACY_CHEM(Y, TEMP_IN, H2O_IN, O2_IN, N2_IN, M, SZA_IN, DTS, NCELL, NSBOXM, NPP, NPC, NTC, NFL)
        IMPLICIT NONE
        
        ! Inputs
        DOUBLE PRECISION, INTENT(INOUT) :: Y(:,:)
        DOUBLE PRECISION, INTENT(IN) :: TEMP_IN(:), H2O_IN(:), O2_IN(:), N2_IN(:), M(:), SZA_IN(:)
        INTEGER, INTENT(IN) :: DTS, NCELL, NSBOXM, NPP, NPC, NTC, NFL
        
        CALL CHEM_ALLOC(NCELL, NSBOXM, NPP, NPC, NTC, NFL)

        ! Populate module-level workspace arrays from dummy arguments.
        ! CHEM_ALLOC zeroed these; they must be set before CHEMCO/DERIV use them.
        TEMP(:) = TEMP_IN(:)
        H2O(:)  = H2O_IN(:)
        O2(:)   = O2_IN(:)
        N2(:)   = N2_IN(:)
...
CONTAINS
    ...
    SUBROUTINE BOXM_STATE_RUN_COARSE_DELTA_CHEM(BOXM_DS, BOXM_STATE, Y_BG_C_PRE)
        CLASS(BOXM_DS_TYPE),     INTENT(IN)    :: BOXM_DS
        ...
        DO CELL_C = 1, BOXM_STATE%NCELL
            ...
            ALLOCATE(TEMP_CELL(BOXM_STATE%NCELL), H2O_CELL(BOXM_STATE%NCELL), O2_CELL(NFINE_CELL), &
                     N2_CELL(BOXM_STATE%NCELL), M_CELL(BOXM_STATE%NCELL), SZA_CELL(NFINE_CELL))

            ! ALLOCATE Y (CONCENTRATION) VARIABLES
            ALLOCATE(Y_BG_F_CELL(BOXM_STATE%NCELL, BOXM_STATE%NSBOXM))
            ALLOCATE(Y_DEL_C_CELL(BOXM_STATE%NCELL, PATCH_STATE%NSBOXM))
            ALLOCATE(Y_TOT_C_CELL(BOXM_STATE%NCELL, BOXM_STATE%NSBOXM))
            ...
            Y_TOT_F_CELL = Y_BG_C_CELL + Y_DEL_C_CELL

            ! Clamp negative totals.  Expected near fresh NO emissions where
            ! O3 titration produces a delta larger in magnitude than the
            ! (evolving) background.  Physically O3 cannot go below zero.
            WHERE (Y_TOT_C_CELL < 0.0_DP) Y_TOT_C_CELL = 0.0_DP

            ! ACCESS Y_TOT_F_CELL(I, :) FOR CHEMISTRY UPDATE
            CALL RUN_LEGACY_CHEM(Y_TOT_C_CELL, TEMP_CELL, H2O_CELL, O2_CELL, N2_CELL, M_CELL, &
                            SZA_CELL, BOXM_STATE%DTS, BOXM_DS%NCELL, BOXM_DS%NSBOXM, BOXM_DS%NPP, &
            ...
            Y_DEL_C_CELL(I,:) = Y_TOT_C_CELL(I,:) - BOXM_STATE%Y_BG_C(CELL_C,:)

            BOXM_STATE%Y_DEL_C(CELL_C,:) = Y_DEL_C_CELL(I,:)

            DEALLOCATE(Y_BG_C_CELL)
            DEALLOCATE(Y_DEL_C_CELL)
            DEALLOCATE(Y_TOT_C_CELL)
...
MODULE BOXM_STATIC_RUN_UTILS
    USE RUN_CHEM_UTILS
    IMPLICIT NONE
    
    PRIVATE
    PUBLIC :: BOXM_STATIC_RUN_INIT, ADVANCE_MET, PROJECT_PLUMES_TO_GRID, RUN_CHEM, BACKPROJECT_GRID_TO_PLUMES
...
CONTAINS
    ...
    SUBROUTINE RUN_CHEM(BOXM_DS, BOXM_STATE, TIME_IDX)
        USE DEFINE_INPUT_TYPES
        USE DEFINE_STATE_TYPES
        USE RUN_CHEM_UTILS
        ...
        IF (BOXM_DS%N_AC > 0) THEN
            CALL BOXM_STATE%RUN_COARSE_DELTA_CHEM(BOXM_DS, Y_BG_C_PRE)
        END IF
...
END MODULE BOXM_STATIC_RUN_UTILS

PROGRAM BOXM_STATIC_RUN
    USE BOXM_STATIC_RUN_UTILS
    USE DEFINE_INPUT_TYPES
    USE DEFINE_STATE_TYPES
    USE DEFINE_OUTPUT_TYPES
    USE VALIDATION_UTILS
    ...
    CALL BOXM_STATIC_RUN_INIT(BOXM_DS, PL_STATE, BOXM_STATE, BOXM_OUT, JOB_ID, DATA_PATH)
    ...
    DO TIME_IDX = 1, BOXM_DS%NTBOXM
        ...
        CALL RESET_STATES(BOXM_DS, BOXM_STATE, TIME_IDX)

        CALL ADVANCE_MET(BOXM_DS, BOXM_STATE, TIME_IDX)

        ! WRITE OUTPUTS at the START of each output interval, before running
        ! chemistry.  This ensures boxm_out[k] = state AFTER k*TS_OUT seconds
        ! of chemistry (boxm_out[0] = IC, aligning with Y.OUT row 0).
        IF (MOD((TIME_IDX-1)*BOXM_DS%TS_SIM, BOXM_DS%TS_OUT) == 0) THEN
            CALL WRITE_OUTPUTS(BOXM_OUTPL_DS, BOXM_DS, PL_STATE, BOXM_STATE, PATCH_STATE, TIME_IDX)
        END IF
        ...
        IF (BOXM_DS%N_AC > 0) THEN
            ...
            IF (BOXM_DS%RUN_CHEM == 1) THEN
                ! UPDATE_CHEMISTRY
                CALL RUN_CHEM(PL_DS, BOXM_DS, BOXM_STATE, PATCH_STATE, TIME_IDX)
            ELSE
                ! Even without chemistry, accumulate fine→coarse deltas so
                ! Y_DEL_C is populated from plume projections.
        ...
        ! === EULERIAN ADVECTION of Y_DEL_C (always, independent of chemistry) ===
