...
CONTAINS
    ...
    SUBROUTINE PL_STATE_BUILD_SLICE_POLYS_M(PL_STATE, SEG_ID, SLICE_ID)
        ...
        ! that are also non-degenerate (emitted, with non-zero spread).
        IF (SEG_ID > 1) THEN
            IF (PL_STATE%FL_ID(SEG_ID-1) == PL_STATE%FL_ID(SEG_ID) .AND. &
                PL_STATE%WP(SEG_ID-1)    == PL_STATE%WP(SEG_ID) - 1 .AND. &
                MAXVAL(PL_STATE%Y_HALF(SEG_ID-1, :)) >= 1.0_DP) THEN
                NPREV = SEG_ID - 1
                HAS_PREV = .TRUE.
            END IF
        END IF
    ...
    SUBROUTINE PL_STATE_BUILD_ACTIVE(PL_STATE, BOXM_DS, BOXM_STATE)
        ...
        DO SEG_ID = 1, PL_STATE%NSEG
            ...
            ! Skip degenerate segments (unemitted/decayed with near-zero spread)
            IF (MAXVAL(PL_STATE%Y_HALF(SEG_ID, :)) < 1.0_DP) CYCLE
            ...
            IF (SEG_ID < PL_STATE%NSEG) THEN
                IF (PL_STATE%FL_ID(SEG_ID+1) == PL_STATE%FL_ID(SEG_ID)) THEN
                    IF (PL_STATE%WP(SEG_ID+1) == PL_STATE%WP(SEG_ID) + 1) THEN
                        IF (MAXVAL(PL_STATE%Y_HALF(SEG_ID+1, :)) >= 1.0_DP) THEN
                            HAS_NEXT_VALID = .TRUE.
                        END IF
                    END IF
                END IF
    ...
    SUBROUTINE PL_STATE_PROJECT_TO_GRID(PL_STATE, PL_DS, BOXM_DS, BOXM_STATE, PATCH_STATE, TIME_IDX)
        ...
        ! Exclude degenerate segments (unemitted/decayed with near-zero spread).
        DO SEG_ID = 1, PL_STATE%NSEG
            SEG_INCLUDED(SEG_ID) = (PL_STATE%ACTIVE_SEG_FLAG(SEG_ID) == 1) .AND. &
                                   (MAXVAL(PL_STATE%Y_HALF(SEG_ID, :)) >= 1.0_DP)
        END DO
        ...
        DO SEG_ID = 1, PL_STATE%NSEG
            ...
            DO SLICE_ID = 1, PL_STATE%NSLICES
                ...
                ! Skip slabs where either face is degenerate (unemitted/decayed segment)
                IF (PL_STATE%Y_HALF(SEG_ID, SLICE_ID) < 1.0_DP) CYCLE
                IF (PL_STATE%Y_HALF(SEG_ID_NEXT, SLICE_ID) < 1.0_DP) CYCLE
    ...
    SUBROUTINE BOXM_STATE_RUN_COARSE_DELTA_CHEM(BOXM_STATE, BOXM_DS, PATCH_STATE)
        ...
        DO CELL_ID = 1, BOXM_STATE%NCELL
            ! Only reconstruct Y_DEL_C for active cells; inactive cells retain
            ! their persistent perturbation from previous timesteps.
            IF (.NOT. BOXM_STATE%ACTIVE_FLAG(CELL_ID)) CYCLE
    ...
    SUBROUTINE BOXM_STATE_ADVECT_Y_DEL_C(BOXM_STATE, BOXM_DS)
        ...
        DO IC = 1, BOXM_STATE%NCELL
            ! Skip active cells: Lagrangian plume handles transport while
            ! segments are alive.  Eulerian advection takes over only after
            ! max plume age (ACTIVE_FLAG becomes .FALSE.).
            IF (BOXM_STATE%ACTIVE_FLAG(IC)) CYCLE
    ...
    SUBROUTINE PATCH_STATE_RUN_FINE_DELTA_CHEM(PATCH_STATE, BOXM_DS, BOXM_STATE, Y_BG_C_PRE)
        ...
        DO CELL_C = 1, BOXM_STATE%NCELL
            ...
            ! Use row existence rather than ACTIVE_FLAG so that fine chemistry
            ! runs on every timestep, not just output steps.  This prevents
            ! Y_DEL_F from going stale while Y_BG_C evolves on intermediate
            ! steps, which was the source of negative Y_TOT.
            ROWS_CELL = PACK((/(I, I=1,PATCH_STATE%NROWS)/), PATCH_STATE%ROW_CELL_C(:) == CELL_C)
            IF (SIZE(ROWS_CELL) == 0) CYCLE
        ...
        IF (BOXM_DS%N_AC > 0) THEN
            ...
            IF (BOXM_STATE%FINE_PLUME_MODE) THEN
                ...
                IF (MOD((TIME_IDX-1)*BOXM_DS%TS_SIM, BOXM_DS%TS_OUT) == 0) THEN
                    ...
                    T_SINCE_PLUME_START_S = REAL((TIME_IDX - PL_DS%TIME_IDX(1)) * BOXM_DS%TS_SIM, DP)
