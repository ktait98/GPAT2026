...
CONTAINS
    ...
    SUBROUTINE ALLOC_2D_REAL(A, N1, N2)
        REAL(DP), ALLOCATABLE, INTENT(INOUT) :: A(:,:)
        INTEGER, INTENT(IN) :: N1, N2
        IF (.NOT. ALLOCATED(A)) THEN
            ALLOCATE(A(N1,N2))
        ELSEIF (SIZE(A,1) /= N1 .OR. SIZE(A,2) /= N2) THEN
            DEALLOCATE(A); ALLOCATE(A(N1,N2))
        END IF
...
MODULE DEFINE_INPUT_TYPES
    ...
    TYPE :: PL_DS_TYPE
        ...
        INTEGER :: FMAX ! MAX FRACTIONAL MASS PER SLICE
...
CONTAINS
    ...
    SUBROUTINE PL_READ_STATIC(PL_DS)
        ...
        ! PL VARS
        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_AGE_S, PL_DS%AGE_S, &
                              START=[1, 1], COUNT=[PL_DS%NTPL, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(age_s)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_LATITUDE, PL_DS%LATITUDE, &
                      START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(latitude)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_LONGITUDE, PL_DS%LONGITUDE, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(longitude)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_LEVEL, PL_DS%LEVEL, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(level)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_ALTITUDE, PL_DS%ALTITUDE, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(altitude)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_WIDTH, PL_DS%WIDTH, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(width)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_DEPTH, PL_DS%DEPTH, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(depth)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_HEADING, PL_DS%HEADING, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(heading)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_SIGMA_YY, PL_DS%SIGMA_YY, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(sigma_yy)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_SIGMA_YZ, PL_DS%SIGMA_YZ, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(sigma_yz)")

        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_SIGMA_ZZ, PL_DS%SIGMA_ZZ, &
                              START=[1, 1, 1], COUNT=[PL_DS%NTPL, PL_DS%HT, PL_DS%NSEG])
...
MODULE DEFINE_STATE_TYPES
    ...
    IMPLICIT NONE
    PRIVATE
    PUBLIC :: PL_SLICES_TYPE, PL_STATE_TYPE, BOXM_STATE_TYPE, PATCH_STATE_TYPE
    ...
    TYPE :: PL_SLICES_TYPE
        ...
        REAL(DP), ALLOCATABLE :: MFRAC(:)      ! (NSLICES)
        REAL(DP), ALLOCATABLE :: W_SLICE(:) ! (NSLICES)

    END TYPE PL_SLICES_TYPE
...
CONTAINS
    ...
    SUBROUTINE PL_STATE_INIT_FROM_PL_DS(PL_STATE, PL_DS)
        ...
        ! ALLOCATE ARRAYS
        IF (.NOT. ALLOCATED(PL_STATE%SEG_ID)) ALLOCATE(PL_STATE%SEG_ID(PL_STATE%NSEG))
        PL_STATE%SEG_ID = [(I, I=1, PL_STATE%NSEG)]
        ...
        IF (.NOT. ALLOCATED(PL_STATE%LONGITUDE)) ALLOCATE(PL_STATE%LONGITUDE(PL_STATE%NSEG, PL_DS%HT))
        IF (.NOT. ALLOCATED(PL_STATE%LATITUDE)) ALLOCATE(PL_STATE%LATITUDE(PL_STATE%NSEG, PL_DS%HT))
        IF (.NOT. ALLOCATED(PL_STATE%ALTITUDE)) ALLOCATE(PL_STATE%ALTITUDE(PL_STATE%NSEG, PL_DS%HT))
        IF (.NOT. ALLOCATED(PL_STATE%LEVEL)) ALLOCATE(PL_STATE%LEVEL(PL_STATE%NSEG, PL_DS%HT))

        IF (.NOT. ALLOCATED(PL_STATE%WIDTH)) ALLOCATE(PL_STATE%WIDTH(PL_STATE%NSEG, PL_DS%HT))
        IF (.NOT. ALLOCATED(PL_STATE%DEPTH)) ALLOCATE(PL_STATE%DEPTH(PL_STATE%NSEG, PL_DS%HT))
        IF (.NOT. ALLOCATED(PL_STATE%HEADING)) ALLOCATE(PL_STATE%HEADING(PL_STATE%NSEG, PL_DS%HT))

        IF (.NOT. ALLOCATED(PL_STATE%SIGMA_YY)) ALLOCATE(PL_STATE%SIGMA_YY(PL_STATE%NSEG, PL_DS%HT))
        IF (.NOT. ALLOCATED(PL_STATE%SIGMA_YZ)) ALLOCATE(PL_STATE%SIGMA_YZ(PL_STATE%NSEG, PL_DS%HT))
        IF (.NOT. ALLOCATED(PL_STATE%SIGMA_ZZ)) ALLOCATE(PL_STATE%SIGMA_ZZ(PL_STATE%NSEG, PL_DS%HT))

        IF (.NOT. ALLOCATED(PL_STATE%PL_MASS)) ALLOCATE(PL_STATE%PL_MASS(PL_STATE%NSEG, PL_STATE%NSPL))

        ! BEFORE SIM START SET EMI_PL_MASS TO PL_MASS
        PL_STATE%PL_MASS(:,:) = PL_DS%EMI_PL_MASS(:,:)

    END SUBROUTINE PL_STATE_INIT_FROM_PL_DS

    SUBROUTINE PL_SLICES_INIT(PL_STATE, PL_SLICES, PL_DS, SEG_ID)
        
        CLASS(PL_SLICES_TYPE), INTENT(INOUT) :: PL_SLICES
        CLASS(PL_STATE_TYPE),  INTENT(IN)    :: PL_STATE
        CLASS(PL_DS_TYPE),    INTENT(IN)    :: PL_DS

        INTEGER :: SLICE, SEG_ID

        PL_SLICES%SEG_ID = SEG_ID
        PL_SLICES%NSLICES = PL_DS%NSLICES
        PL_SLICES%FMAX = PL_DS%FMAX

        IF (.NOT. ALLOCATED(PL_SLICES%Y_HALF)) ALLOCATE(PL_SLICES%Y_HALF(PL_STATE%NSEG, 2, PL_SLICES%NSLICES))
        IF (.NOT. ALLOCATED(PL_SLICES%Z_HALF)) ALLOCATE(PL_SLICES%Z_HALF(PL_STATE%NSEG, 2, PL_SLICES%NSLICES))
        IF (.NOT. ALLOCATED(PL_SLICES%W_SLICE)) ALLOCATE(PL_SLICES%W_SLICE(PL_SLICES%NSLICES))
        IF (.NOT. ALLOCATED(PL_SLICES%MFRAC)) ALLOCATE(PL_SLICES%MFRAC(PL_SLICES%NSLICES))

        ! DEFINE CUMULATIVE MASS LADDER
        DO SLICE = 1, PL_SLICES%NSLICES
            PL_SLICES%MFRAC(SLICE) = PL_SLICES%FMAX * REAL(SLICE, DP) / REAL(PL_SLICES%NSLICES, DP)
        END DO

        ! CALC WEIGHTS ARRAY
        PL_SLICES%W_SLICE(1) = PL_SLICES%MFRAC(1)
        DO SLICE = 2, PL_SLICES%NSLICES
            PL_SLICES%W_SLICE(SLICE) = PL_SLICES%MFRAC(SLICE) - PL_SLICES%MFRAC(SLICE-1)
        END DO

        ! OPTIONAL STRICT CONSERVATION ADJUSTMENT
        PL_SLICES%W_SLICE(PL_SLICES%NSLICES) = PL_SLICES%W_SLICE(PL_SLICES%NSLICES) + &
            (1.0_DP - PL_SLICES%FMAX)

    END SUBROUTINE PL_SLICES_INIT
        
    SUBROUTINE PL_STATE_ADVANCE_GEOM(PL_STATE, PL_DS, PL_SLICES, TIME_IDX)
        CLASS(PL_STATE_TYPE),    INTENT(INOUT) :: PL_STATE
        CLASS(PL_SLICES_TYPE),   INTENT(INOUT) :: PL_SLICES
        CLASS(PL_DS_TYPE),       INTENT(IN)    :: PL_DS

        INTEGER, INTENT(IN) :: TIME_IDX
        INTEGER :: SEG_ID, HT, SLICE
        REAL(DP) :: SIGMA_Y, SIGMA_Z
        REAL(DP) :: F, U
        REAL(DP), PARAMETER :: F_EPS = 1.0E-12_DP

        ! GRAB PLUME GEOMETRY FROM PL DS
        PL_STATE%LONGITUDE(:,:) = PL_DS%LONGITUDE(:,:,TIME_IDX) ! TAIL (HT=1)
        PL_STATE%LATITUDE(:,:)  = PL_DS%LATITUDE(:,:,TIME_IDX)  ! (NSEG, HT)
        PL_STATE%ALTITUDE(:,:)  = PL_DS%ALTITUDE(:,:,TIME_IDX)
        PL_STATE%LEVEL(:,:)     = PL_DS%LEVEL(:,:,TIME_IDX)

        PL_STATE%WIDTH(:,:)     = PL_DS%WIDTH(:,:,TIME_IDX)
        PL_STATE%DEPTH(:,:)     = PL_DS%DEPTH(:,:,TIME_IDX)
        PL_STATE%HEADING(:,:)   = PL_DS%HEADING(:,:,TIME_IDX)
        PL_STATE%SIGMA_YY(:,:)  = PL_DS%SIGMA_YY(:,:,TIME_IDX)
        PL_STATE%SIGMA_YZ(:,:)  = PL_DS%SIGMA_YZ(:,:,TIME_IDX)
        PL_STATE%SIGMA_ZZ(:,:)  = PL_DS%SIGMA_ZZ(:,:,TIME_IDX)

        ! BUILD PLUME SEGMENT REPRESENTATION FROM SLICES
        PL_SLICES%Y_HALF(:,:,:) = 0.0_DP
        PL_SLICES%Z_HALF(:,:,:) = 0.0_DP

        DO SEG_ID = 1, PL_STATE%NSEG
            DO HT = 1, 2

                ! SIGMA_YY, SIGMA_ZZ ARE VARIANCES IN YOUR STATE
                SIGMA_Y = SQRT( MAX( PL_STATE%SIGMA_YY(SEG_ID, HT), 1.0E-30_DP ) )
                SIGMA_Z = SQRT( MAX( PL_STATE%SIGMA_ZZ(SEG_ID, HT), 1.0E-30_DP ) )

                DO SLICE = 1, PL_SLICES%NSLICES

                    F  = PL_SLICES%MFRAC(SLICE)
                    ...
                    PL_SLICES%Y_HALF(SEG_ID, HT, SLICE) = SQRT(2.0_DP) * SIGMA_Y * ERFINV(U)
                    PL_SLICES%Z_HALF(SEG_ID, HT, SLICE) = SQRT(2.0_DP) * SIGMA_Z * ERFINV(U)
                END DO
            END DO
        END DO
    ...
    SUBROUTINE PL_STATE_BUILD_ACTIVE(PL_STATE, BOXM_STATE)
        ...
        DO SEG_ID = 1, PL_STATE%NSEG
            ...
            PL_LON = PL_STATE%LONGITUDE(SEG_ID, HT)
            PL_LAT = PL_STATE%LATITUDE(SEG_ID, HT)
            PL_ALT = PL_STATE%ALTITUDE(SEG_ID, HT)
    ...
    SUBROUTINE PL_STATE_PROJECT_TO_GRID(PL_STATE, PL_SLICES, BOXM_DS, BOXM_STATE, PATCH_STATE)
        CLASS(PL_STATE_TYPE),    INTENT(INOUT) :: PL_STATE
        CLASS(PL_SLICES_TYPE),   INTENT(INOUT) :: PL_SLICES
        CLASS(BOXM_DS_TYPE),     INTENT(IN)    :: BOXM_DS
        CLASS(BOXM_STATE_TYPE),  INTENT(INOUT) :: BOXM_STATE
        ...
        DO SEG_ID = 1, PL_STATE%NSEG
            ...
            DO SLICE_ID = 1, PL_SLICES%NSLICES
                Z_HALF_TAIL = PL_SLICES%Z_HALF(SEG_ID, 1, SLICE_ID)
                Z_HALF_HEAD = PL_SLICES%Z_HALF(SEG_ID, 2, SLICE_ID)
                W_HALF_M = 0.5_DP * (PL_SLICES%Y_HALF(SEG_ID, 1, SLICE_ID) + PL_SLICES%Y_HALF(SEG_ID, 2, SLICE_ID))
        ...
        IF (.NOT. ALLOCATED(PATCH_STATE%TIME_IDX)) THEN
            ALLOCATE(PATCH_STATE%TIME_IDX(PATCH_STATE%NROWS))
        ELSEIF (SIZE(PATCH_STATE%TIME_IDX) /= PATCH_STATE%NROWS) THEN
            DEALLOCATE(PATCH_STATE%TIME_IDX)
            ALLOCATE(PATCH_STATE%TIME_IDX(PATCH_STATE%NROWS))
        END IF
                ...
                DO I_ALT = I_ALT_MIN, I_ALT_MAX
                    ...
                    DO I_LAT = I_LAT_MIN, I_LAT_MAX
                        ...
                        DO I_LON = I_LON_MIN, I_LON_MAX
                            ...
                            PATCH_STATE%TIME_IDX(ROW) = PL_STATE%TIME_IDX

                            DO SPECIES_ID = 1, PL_STATE%NSPL
                                TOTAL_MASS = PL_STATE%PL_MASS(SEG_ID, SPECIES_ID)
                                MASS_SLICE = TOTAL_MASS * PL_SLICES%W_SLICE(SLICE_ID) * ALPHA
    ...
    CONTAINS
        SUBROUTINE BUILD_RECT_POLY(L_HALF, W_HALF, HEAD_RAD, LAT0, LON0, M_PER_DEG_LAT, M_PER_DEG_LON, &
                                   X_OUT, Y_OUT, N_OUT, LAT_MIN, LAT_MAX, LON_MIN, LON_MAX)
            REAL(DP), INTENT(IN) :: L_HALF, W_HALF, HEAD_RAD, LAT0, LON0, M_PER_DEG_LAT, M_PER_DEG_LON
            ...
            X_C(1) =  UX * L_HALF + VX * W_HALF
            Y_C(1) =  UY * L_HALF + VY * W_HALF
            X_C(2) =  UX * L_HALF - VX * W_HALF
            Y_C(2) =  UY * L_HALF - VY * W_HALF
            X_C(3) = -UX * L_HALF - VX * W_HALF
            Y_C(3) = -UY * L_HALF - VY * W_HALF
            X_C(4) = -UX * L_HALF + VX * W_HALF
            Y_C(4) = -UY * L_HALF + VY * W_HALF
...
CONTAINS
    ...
    ! ... PL_OUT METHODS ...
    SUBROUTINE PL_OUT_INIT(PL_OUT, FILEPATH)
        ...
        STATUS = NF90_INQ_DIMID(PL_OUT%PL_OUT_NCID, "species_out", PL_OUT%DIMID_SPECIES_OUT)
        CALL NC_CHECK(STATUS, "NF90_INQ_DIMID(species_out)")

        STATUS = NF90_INQUIRE_DIMENSION(PL_OUT%PL_OUT_NCID, PL_OUT%DIMID_SPECIES_OUT, LEN=PL_OUT%NSOUT)
        ...
        STATUS = NF90_INQ_VARID(PL_OUT%PL_OUT_NCID, "species_out_num", PL_OUT%VARID_SPECIES_OUT_NUM)
    ...
    SUBROUTINE PL_OUT_READ_STATIC(PL_OUT)
        CLASS(PL_OUT_TYPE), INTENT(INOUT) :: PL_OUT
        INTEGER :: STATUS
        ...
        IF (PL_OUT%NSOUT <= 0) STOP "PL_OUT_READ_STATIC: NSOUT NOT SET"
        ...
        IF (.NOT. ALLOCATED(PL_OUT%SPECIES_OUT_NUM)) ALLOCATE(PL_OUT%SPECIES_OUT_NUM(PL_OUT%NSOUT))
        IF (.NOT. ALLOCATED(PL_OUT%PL_MASS)) ALLOCATE(PL_OUT%PL_MASS(PL_OUT%NSEG, PL_OUT%NSOUT, PL_OUT%NTOUT))
        ...
        STATUS = NF90_GET_VAR(PL_OUT%PL_OUT_NCID, PL_OUT%VARID_SPECIES_OUT_NUM, PL_OUT%SPECIES_OUT_NUM)
        CALL NC_CHECK(STATUS, "NF90_GET_VAR(species_out_num)")

    END SUBROUTINE PL_OUT_READ_STATIC

    SUBROUTINE PL_OUT_WRITE(PL_OUT, PL_STATE, TIME_IDX)
        CLASS(PL_OUT_TYPE), INTENT(INOUT) :: PL_OUT
        CLASS(PL_STATE_TYPE), INTENT(IN)  :: PL_STATE
        ...
        IF (PL_OUT%NSOUT <= 0) STOP "PL_OUT_WRITE: NSOUT NOT SET"
        ...
        ! IF TIME_IDX(I) == PL_OUT%TIME_IDX WRITE PL OUT VARS
        IF (.NOT. ALLOCATED(PL_STATE%PL_MASS)) STOP "PL_OUT_WRITE: PL_STATE%PL_MASS NOT ALLOCATED"

        OUT_I = MINLOC(ABS(PL_OUT%TIME_IDX - TIME_IDX), 1)

        IF (PL_OUT%TIME_IDX(OUT_I) == TIME_IDX) THEN
            STATUS = NF90_PUT_VAR(PL_OUT%PL_OUT_NCID, PL_OUT%VARID_PL_MASS, PL_STATE%PL_MASS, &
                                  START=[OUT_I, 1, 1], COUNT=[1, PL_OUT%NSOUT, PL_OUT%NSEG])
    ...
    SUBROUTINE BOXM_OUT_WRITE(BOXM_OUT, BOXM_STATE, TIME_IDX)
        ...
        IF (BOXM_STATE%TIME_IDX < BOXM_OUT%TIME_IDX(1) .OR. BOXM_STATE%TIME_IDX > BOXM_OUT%TIME_IDX(BOXM_OUT%NTOUT)) &
        STOP "BOXM_OUT_WRITE: BOXM_STATE TIME_IDX OUT OF RANGE"
        ...
        IF BOXM_OUT%TIME_IDX(OUT_I) == TIME_IDX THEN
            ...
            STATUS = NF90_PUT_VAR(BOXM_OUT%BOXM_OUT_NCID, BOXM_OUT%VARID_Y_BG_C, BOXM_STATE%Y_BG_C, &
                                START=[BOXM_STATE%TIME_IDX, 1, 1], COUNT=[1, BOXM_OUT%NSOUT, BOXM_OUT%NCELL])
            ...
            STATUS = NF90_PUT_VAR(BOXM_OUT%BOXM_OUT_NCID, BOXM_OUT%VARID_ACTIVE_FLAG, MERGE(1, 0, BOXM_STATE%ACTIVE_FLAG), &
                                START=[BOXM_STATE%TIME_IDX, 1], COUNT=[1, BOXM_OUT%NCELL])
    ...
    SUBROUTINE PATCH_TABLE_WRITE(PATCH_TABLE, PATCH_STATE)
        CLASS(PATCH_TABLE_TYPE), INTENT(INOUT) :: PATCH_TABLE
        ...
        IF (PATCH_TABLE%NROWS <= 0) STOP "PATCH_TABLE_WRITE: PATCH_STATE NROWS NOT SET"

        ! WRITE PATCH TABLE VARS
        IF (.NOT. ALLOCATED(PATCH_STATE%Y_DEL_F)) STOP "PATCH_TABLE_WRITE: PATCH_STATE%Y_DEL_F NOT ALLOCATED"
        ...
        IF PATCH_TABLE%TIME_IDX(OUT_I) == TIME_IDX THEN
            STATUS = NF90_PUT_VAR(PATCH_TABLE%PATCH_TABLE_NCID, PATCH_TABLE%VARID_Y_DEL_F, PATCH_STATE%Y_DEL_F, &
                            START=[PATCH_STATE%ROW_IDX, 1], COUNT=[PATCH_STATE%NROWS, PATCH_TABLE%NSOUT])
...
CONTAINS
    SUBROUTINE BOXM_RUN_INIT(FL_DS, PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, &
    ...
    SUBROUTINE PROJECT_PLUMES_TO_GRID(PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, TIME_IDX)
        USE DEFINE_INPUT_TYPES
        USE DEFINE_STATE_TYPES
        IMPLICIT NONE
        ...
        CALL PL_STATE%ADVANCE_GEOM(PL_DS, PL_SLICES, TIME_IDX)
        CALL PL_STATE%BUILD_ACTIVE(BOXM_STATE)
        CALL PL_STATE%PROJECT_TO_GRID(PL_SLICES, BOXM_DS, BOXM_STATE, PATCH_STATE)
    ...
    SUBROUTINE BACKPROJECT_GRID_TO_PLUMES(PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, TIME_IDX)
        USE DEFINE_INPUT_TYPES
        USE DEFINE_STATE_TYPES
        IMPLICIT NONE
    ...
    SUBROUTINE WRITE_OUTPUTS(PL_OUT, BOXM_OUT, PATCH_TABLE, PL_STATE, BOXM_STATE, PATCH_STATE, TIME_IDX)
        USE DEFINE_OUTPUT_TYPES
        USE DEFINE_STATE_TYPES
        IMPLICIT NONE
        ...
        CALL PL_OUT%WRITE(PL_STATE, TIME_IDX)
...
PROGRAM BOXM_RUN
    ...
    TYPE(FL_DS_TYPE),       INTENT(OUT) :: FL_DS
    TYPE(PL_DS_TYPE),       INTENT(OUT) :: PL_DS
    TYPE(BOXM_DS_TYPE),     INTENT(OUT) :: BOXM_DS

    TYPE(PL_STATE_TYPE),    INTENT(OUT) :: PL_STATE
    TYPE(PL_SLICES_TYPE),   INTENT(OUT) :: PL_SLICES
    TYPE(BOXM_STATE_TYPE),  INTENT(OUT) :: BOXM_STATE
    TYPE(PATCH_STATE_TYPE), INTENT(OUT) :: PATCH_STATE

    TYPE(PL_OUT_TYPE),      INTENT(OUT) :: PL_OUT
    TYPE(BOXM_OUT_TYPE),    INTENT(OUT) :: BOXM_OUT
    TYPE(PATCH_TABLE_TYPE), INTENT(OUT) :: PATCH_TABLE

    CHARACTER(LEN=*),      INTENT(IN)  :: JOB_ID
    CHARACTER(LEN=*),      INTENT(IN)  :: DATA_PATH

    ! RETREIVE JOB ID FROM COMMAND LINE ARG
    CALL GETARG(1, JOB_ID)
    CALL GETARG(2, DATA_PATH)

    CALL BOXM_RUN_INIT(FL_DS, PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, &
                       PL_OUT, BOXM_OUT, PATCH_TABLE, JOB_ID, DATA_PATH)

    ! MAIN SIMULATION LOOP OVER BOXM TIME STEPS
    DO TIME_IDX = 1, BOXM_DS%NTIME
        
        ! PROJECT_PLUMES_TO_GRID
        CALL PROJECT_PLUMES_TO_GRID(PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, TIME_IDX)

        ! UPDATE_CHEMISTRY
        CALL RUN_CHEM(BOXM_DS, BOXM_STATE, PATCH_STATE, TIME_IDX)

        ! BACKPROJECT_GRID_TO_PLUMES
        CALL BACKPROJECT_GRID_TO_PLUMES(PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, TIME_IDX)

        ! WRITE OUTPUTS
        CALL WRITE_OUTPUTS(PL_OUT, BOXM_OUT, PATCH_TABLE, PL_STATE, BOXM_STATE, PATCH_STATE, TIME_IDX)
