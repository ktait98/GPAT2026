...
CONTAINS
    ...
    SUBROUTINE PL_READ_STATIC(PL_DS)
        ...
        ! PL VARS
        STATUS = NF90_GET_VAR(PL_DS%PL_NCID, PL_DS%VARID_ACTIVE_SEG_FLAG, PL_DS%ACTIVE_SEG_FLAG, &
                              START=[1, 1], COUNT=[PL_DS%NSEG, PL_DS%NTPL])
...
MODULE DEFINE_STATE_TYPES
    ...
    TYPE :: PL_STATE_TYPE
        ...
        REAL(DP), ALLOCATABLE :: SLICE_POLY(:,:,:,:,:) ! (NSEG, NSLICES, 2, 4, 3) PL_SLICES_POLY(SEG_ID, SLICE_ID, HT, CORNER_ID, COORD)
...
CONTAINS
    ...
    SUBROUTINE PL_SLICES_INIT(PL_STATE, PL_DS, SEG_ID)
        ...
        ! DO SLICE_ID = 1, PL_STATE%NSLICES
        !     CALL CALC_SLICE_POLY(PL_STATE%SLICE_POLY(:, SLICE_ID, :, :, :), &
        !                           PL_STATE%Y_HALF(:, :, SLICE_ID), &
        !                           PL_STATE%Z_HALF(:, :, SLICE_ID), &
        !                           PL_STATE%WIDTH, PL_STATE%DEPTH, PL_STATE%HEADING)
        ! END DO
    ...
    SUBROUTINE CALC_SLICE_POLY(PL_STATE, SEG_ID, SLICE_ID)
        ...
        REAL(DP) :: Y_HALF(2), Z_HALF(2)
        REAL(DP) :: WIDTH(2), DEPTH(2), HEADING(2)
        REAL(DP) :: LONGITUDE(2), LATITUDE(2), ALTITUDE(2)

        LONGITUDE(:) = PL_STATE%LONGITUDE(SEG_ID, :)
        LATITUDE(:) = PL_STATE%LATITUDE(SEG_ID, :)
        ALTITUDE(:) = PL_STATE%ALTITUDE(SEG_ID, :)

        Y_HALF(:) = PL_STATE%Y_HALF(SEG_ID, :, SLICE_ID)
        Z_HALF(:) = PL_STATE%Z_HALF(SEG_ID, :, SLICE_ID)
        HEADING(:) = PL_STATE%HEADING(SEG_ID, :)

        ! CALC POLY CORNERS FOR THIS SEGMENT AND SLICE
        ! CALL CALC_PLUME_SLICE_POLY(Y_HALF, Z_HALF, WIDTH, DEPTH, HEADING, &
        !                            PL_STATE%SLICE_POLY(SEG_ID, SLICE_ID, :, :, :))

    END SUBROUTINE CALC_SLICE_POLY
        
    SUBROUTINE PL_STATE_ADVANCE_GEOM(PL_STATE, PL_DS, TIME_IDX)
        CLASS(PL_STATE_TYPE),    INTENT(INOUT) :: PL_STATE
        CLASS(PL_DS_TYPE),       INTENT(IN)    :: PL_DS

        INTEGER, INTENT(IN) :: TIME_IDX
        INTEGER :: SEG_ID, HT, SLICE_ID, PL_I, I
            ...
            IF (PL_STATE%ACTIVE_SEG_FLAG(SEG_ID) == 1) THEN
                ...
                DO SLICE_ID = 1, PL_STATE%NSLICES
                    DO HT = 1, 2
                        ...
                        PL_STATE%Y_HALF(SEG_ID, HT, SLICE_ID) = SQRT(2.0_DP) * SIGMA_Y * ERFINV(U)
                        PL_STATE%Z_HALF(SEG_ID, HT, SLICE_ID) = SQRT(2.0_DP) * SIGMA_Z * ERFINV(U)

                        IF (PL_STATE%Y_HALF(SEG_ID, HT, SLICE_ID) > 0.0_DP) THEN
                            PRINT *, "Y_HALF is non-zero: ", PL_STATE%Y_HALF(SEG_ID, HT, SLICE_ID)
                        END IF

                        IF (PL_STATE%Z_HALF(SEG_ID, HT, SLICE_ID) > 0.0_DP) THEN
                            PRINT *, "Z_HALF is non-zero: ", PL_STATE%Z_HALF(SEG_ID, HT, SLICE_ID)
                        END IF
                    END DO
...
CONTAINS
    ...
    SUBROUTINE PL_OUT_WRITE(PL_OUT, PL_STATE, PL_DS, TIME_IDX)
        ...
        INTEGER, INTENT(IN) :: TIME_IDX
        INTEGER :: STATUS, PL_I, OUT_I, I, NDIMS, DLEN, SPECIES_ID, SLICE_ID, HT_ID
        ...
        IF (OUT_I > 0) THEN
            ...
            IF (PL_DS%OUTPUT_PL_SLICES == 1) THEN
                ! Y_HALF, Z_HALF, M_FRAC, W_SLICE
                PL_OUT%M_FRAC(:) = PL_STATE%M_FRAC(:)
                PL_OUT%W_SLICE(:) = PL_STATE%W_SLICE(:)

                ! PRINT *, "WRITING PL OUT FOR TIME_IDX=", TIME_IDX, " OUT_I=", OUT_I, " PL_I=", PL_I
...
CONTAINS
    ...
    SUBROUTINE PROJECT_PLUMES_TO_GRID(FL_DS, PL_DS, BOXM_DS, PL_STATE, BOXM_STATE, PATCH_STATE, TIME_IDX)
        ...
        INTEGER, INTENT(IN) :: TIME_IDX
        CALL PL_STATE%ADVANCE_GEOM(PL_DS, TIME_IDX)
