...
CONTAINS
    ...
    SUBROUTINE PL_STATE_ADVANCE_GEOM(PL_STATE, PL_SLICES, PL_DS, TIME_IDX)
        CLASS(PL_STATE_TYPE),    INTENT(INOUT) :: PL_STATE
        CLASS(PL_SLICES_TYPE),   INTENT(INOUT) :: PL_SLICES
        CLASS(PL_DS_TYPE),       INTENT(IN)    :: PL_DS
    ...
    SUBROUTINE PL_STATE_ADVANCE_MASS(PL_STATE, PL_SLICES, PL_DS, TIME_IDX)
        CLASS(PL_STATE_TYPE),    INTENT(INOUT) :: PL_STATE
        CLASS(PL_SLICES_TYPE),   INTENT(INOUT) :: PL_SLICES
        CLASS(PL_DS_TYPE),       INTENT(IN)    :: PL_DS

        INTEGER :: SEG_ID, SLICE, SPECIES_ID, TIME_IDX
        REAL(DP) :: MASS_SEG
        ...
        ! AT PLUME TIME 1 INITIALIZE MASS FROM PL_DS
        IF (TIME_IDX == PL_DS%TIME_IDX(1)) THEN
            PL_STATE%PL_MASS(:,:) = PL_DS%EMI_PL_MASS(:,:)
        END IF

        ! DURING PLUME TIME, UPDATE MASS FROM BACKPROJECTION
...
CONTAINS
    ...
    SUBROUTINE PL_OUT_WRITE(PL_OUT, PL_STATE, TIME_IDX)
        ...
        OUT_I = TIME_IDX - PL_OUT%TIME_IDX(1) + 1

        IF (ANY(TIME_IDX == PL_OUT%TIME_IDX)) THEN
...
MODULE BOXM_RUN_UTILS
    IMPLICIT NONE
    PRIVATE
    PUBLIC :: BOXM_RUN_INIT, PROJECT_PLUMES_TO_GRID, RUN_CHEM, BACKPROJECT_GRID_TO_PLUMES, WRITE_OUTPUTS, CLOSE_DATASETS
CONTAINS
    SUBROUTINE BOXM_RUN_INIT(FL_DS, PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, &
        ...
        CALL PL_STATE%PL_SLICES_INIT(PL_SLICES, PL_DS, SEG_ID=1)
    ...
    SUBROUTINE PROJECT_PLUMES_TO_GRID(PL_DS, BOXM_DS, PL_STATE, PL_SLICES, BOXM_STATE, PATCH_STATE, TIME_IDX)
        ...
        CALL PL_STATE%ADVANCE_GEOM(PL_SLICES, PL_DS, TIME_IDX)
        CALL PL_STATE%ADVANCE_MASS(PL_SLICES, PL_DS, TIME_IDX)
