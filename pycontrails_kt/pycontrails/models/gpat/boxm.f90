...
MODULE DEFINE_STATE_TYPES
    ...
    TYPE :: PL_STATE_TYPE
        ...
        REAL(DP), ALLOCATABLE :: DEL_PL_MASS(:,:)   ! (NSEG, NSPL) kg
    ...
    TYPE :: PL_SLICES_TYPE
        ...
        REAL(DP), ALLOCATABLE :: SLICE_VERTICES(:,:,:) ! (NSEG, NSLICES, 3)
        REAL(DP), ALLOCATABLE :: SLICE_WEIGHTS(:,:)  ! (NSEG, NSLICES)
    END TYPE PL_SLICES_TYPE

CONTAINS
    ! ---------- PL STATE METHODS ----------
    SUBROUTINE PL_STATE_INIT_FROM_PL_DS(PL_STATE, PL_DS, TIME_IDX)
        ...
        IF (.NOT. ALLOCATED(PL_STATE%DEL_PL_MASS)) ALLOCATE(PL_STATE%DEL_PL_MASS(PL_STATE%NSPL, PL_STATE%NSEG))

        ! AT TIMESTEP 1 SET EMI_PL_MASS TO DEL_PL_MASS
        PL_STATE%DEL_PL_MASS(:,:) = PL_DS%EMI_PL_MASS(:,:)
    ...
    SUBROUTINE PL_STATE_ADVANCE_GEOM(PL_STATE, PL_DS, PL_SLICES, TIME_IDX)
        CLASS(PL_STATE_TYPE), INTENT(INOUT) :: PL_STATE
        CLASS(PL_DS_TYPE),    INTENT(IN)    :: PL_DS
        CLASS(PL_SLICES_TYPE), INTENT(IN)   :: PL_SLICES

        INTEGER, INTENT(IN) :: TIME_IDX

        ! GRAB PLUME GEOMETRY FROM PL DS
        PL_STATE%LONGITUDE(:) = PL_DS%LONGITUDE(:,TIME_IDX)
        PL_STATE%LATITUDE(:) = PL_DS%LATITUDE(:,TIME_IDX)
        ...
        PL_SLICES%SLICE_VERTICES(:,:,:) = 0.0_DP
        PL_SLICES%SLICE_WEIGHTS(:,:) = 0.0_DP

        DO SLICE 


        ! ATTACH VERTEX MATRIX TO PLUME STATE

        
    END SUBROUTINE PL_STATE_ADVANCE_GEOM

    SUBROUTINE PL_STATE_CALC_SLICE_WEIGHTS(PL_STATE, PL_SLICES)
        CLASS(PL_STATE_TYPE), INTENT(INOUT) :: PL_STATE

    END SUBROUTINE PL_STATE_CALC_SLICE_WEIGHTS
...
MODULE DEFINE_OUTPUT_TYPES
    ...
    TYPE :: PL_OUT_TYPE
        ...
        ! PL OUT VARS
        REAL(DP), ALLOCATABLE :: DEL_PL_MASS(:,:,:)
        ...
        INTEGER, PRIVATE :: VARID_DEL_PL_MASS = -1
...
CONTAINS
    ...
    SUBROUTINE PL_OUT_INIT(PL_OUT, FILEPATH)
        ...
        STATUS = NF90_INQ_VARID(PL_OUT%PL_OUT_NCID, "del_pl_mass", PL_OUT%VARID_DEL_PL_MASS)
        CALL NC_CHECK(STATUS, "NF90_INQ_VARID(del_pl_mass)")
    ...
    SUBROUTINE PL_OUT_READ_STATIC(PL_OUT)
        ...
        IF (.NOT. ALLOCATED(PL_OUT%DEL_PL_MASS)) ALLOCATE(PL_OUT%DEL_PL_MASS(PL_OUT%NSEG, PL_OUT%NSOUT, PL_OUT%NTOUT))
    ...
    SUBROUTINE PL_OUT_WRITE(PL_OUT, PL_STATE)
        CLASS(PL_OUT_TYPE), INTENT(INOUT) :: PL_OUT
        ...
        IF (PL_STATE%TIME_IDX < PL_OUT%TIME_IDX(1) .OR. PL_STATE%TIME_IDX > PL_OUT%TIME_IDX(PL_OUT%NTOUT)) &
        STOP "PL_OUT_WRITE: PL_STATE TIME_IDX OUT OF RANGE"

        ! WRITE PL OUT VARS
        STATUS = NF90_PUT_VAR(PL_OUT%PL_OUT_NCID, PL_OUT%VARID_DEL_PL_MASS, PL_STATE%DEL_PL_MASS, &
                              START=[PL_STATE%TIME_IDX, 1, 1], COUNT=[1, PL_OUT%NSOUT, PL_OUT%NSEG])
        CALL NC_CHECK(STATUS, "NF90_PUT_VAR(del_pl_mass)")
    ...
    SUBROUTINE BOXM_OUT_WRITE(BOXM_OUT, BOXM_STATE)
        CLASS(BOXM_OUT_TYPE), INTENT(INOUT) :: BOXM_OUT
...
PROGRAM BOXM_RUN
    ...
    CHARACTER(LEN=256) :: JOB_ID, DATA_PATH
